...
use Yiisoft\Rbac\Factory\DIRuleFactory;
...
abstract class DbManagerTestCase extends ManagerTestCase
{
    ...
    public function testCheckAccessCache()
    {
        ...
        $this->auth->userHasPermission('author B', 'readPost');
        $this->auth->userHasPermission(new UserID('author B'), 'createPost');
        ...
        foreach (['readPost' => true, 'createPost' => false] as $permission => $result) {
            $this->assertEquals($result, $this->auth->userHasPermission('reader A', $permission), "Checking $permission");
        }
